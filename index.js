...
function customEntries(obj) {
  ...
  const custom = Object.entries(obj);
...
function customObjectFromEntries(entries) {
  ...
  const custom = entries.map(([key, val]) => {
  ...
  const from = Object.fromEntries(custom);
...
function checkProperty(obj, prop) {
  ...
  const check = obj.hasOwnProperty(prop);
...
function filterObjectsByKey(arr, key) {
  ...
  const a = arr.filter((obj) => obj.hasOwnProperty(key));
...
function checkAndPreventExtensions(obj) {
  ...
  const check = Object.preventExtensions(obj);
  // Якщо можна, запобігаємо подальшому додаванню властивостей
  const i = Object.isExtensible(check);
...
function convertArrayToObj(arr) {
  ...
  const b = Object.fromEntries(arr);
  return b;
}
